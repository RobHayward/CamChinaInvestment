...
& 	\multicolum{3}{c}{Portfolio weights} &\\
...
\end{frame
		\pause
	\item Weighting assets according to capitalisation
		\begin{itemize}
			\item it may be difficult to determine the size of the market
			\item what is the investment universe?
		\end{itemize}
	\item Optimising time series according to mean-variance
		\begin{itemize}
			\item requires historic data for returns
		\end{itemize}
\end{itemize}
\end{frame}
