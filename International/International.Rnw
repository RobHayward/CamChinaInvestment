...
\item Questions about asset and indistry allocation are combined with those of region or country
...
\begin{itemize}[<+-| alert@+>]
		\pause
	\item Lower risk can be achievde for same return with the use of international diversification
...
\end{center}
\end{table}
December 1989 estmated holdings (French and Poterba, 1991).  Weight is domestic stock market weight as percentage of big five
\end{frame}	
...
\begin{itemize}[<+-| alert@+>]
	...
	\item Exchange rate can be hedged
		\begin{itemize}
			\item active or passive investment (evidence suggest thta active investment is not successful
		\end{itemize}
...
\begin{itemize}[<+-| alert@+>]
\pause
\item Opening up of countries to trade has brought economic cycles more into synchroisation
\item Opening of capital markets has allowed capital to flow freely from one country to another. 
\item Struture is more \emph{risk-on/risk-off}
\end{itemize}
\end{frame}
...
\begin{frame}{Industry vs Country}
	Marcelo, Quiros and Martins (2013) study industry-country effects
\begin{itemize}[<+-| alert@+>]
		\pause
	\item 
\end{itemize}
...
\begin{itemize}[<+-| alert@+>]
	...
	\item Fundamental factors are more local and hetrogenos
\end{itemize}
\end{frame}
