...
\tikzstyle{shape1} = [circle, minimum width = 2.8cm, fill = brown!40, text centered, 
...
\begin{itemize}[<+-| alert@+>]
	...
	\item Crisis: where there is a normal distribution with a very large standard deviation
...
\item Are the anomolies or effects (value, growth etc) inefficiencies or risk factors? 
\end{itemize}
\end{frame}
...
EFA & iShares MSCI EAFE & 71m & 24b\\
VEA & Vanguard FT Developed & 69b & 8m\\
VWO & Vanguard Emerging & 59m & 13b
\end{tabular}
\end{center}
\end{table}
End of 2017
...
\item fees
\end{itemize}
	\item They are an efficient vehicle for retail and professonal investors
		\begin{itemize}
     \item they allow cheap access to a wide variety of diversified investments
			\item their liquidity allows institutions to take large positions even when underlyng liquidity may be more limited
		\end{itemize}
\end{itemize}
\end{frame}
...
\begin{frame}{Effective and efficient Vehicle}
	...
	\pause
\begin{itemize}[<+-| alert@+>]
			\item As an open-ened invstment vehicle new shares can be created to ensure that ETF price is close to the Net Asset Value (NAV)
			...
			\item The strucutre means that dividends can be re-invested and shares can be lent to other institutions to enhance income
		\begin{itemize} 
...
\begin{itemize}[<+-| alert@+>]
	...
	\item Synthetic ETF: the Authorised Participant will provide cash in exchange for shares
		\begin{itemize}
			...
			\item artbitrage is more complex
...
% Secondary Market
\node at (6, 0.8) {\textbf{SECONDARY MARKET}}; 
...
\begin{itemize}[<+-| alert@+>]
	...
	\item The ETF may have more liquidity than the underlying (particuarly in the corporate sector)
	\item There is some concern about the techniques that have been used to create bond indices
		\begin{itemize}
			\item Countries with the largest amount of outsanding bonds have the largest amount of debt
		\end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{Other Assets: Commodities}
	Commofity  ETF have been established
	\pause
\begin{itemize}[<+-| alert@+>]
	\item These are generally synthetic ETF, using futures to gain exposure to the underying assets
	...
	\item Currency ETF tend to hold money market products
\end{itemize}
\end{frame}

\begin{frame}{Positive and negative multiples}
	The first of these ETF to promise positive or negaive multiples of an index was created in 2006 in the US. 
\begin{itemize}[<+-| alert@+>]
		\pause
	...
	\item Much smaller amounts are traded (though the multiple means that smaller amonts can generate the same return)
	\item They promise to track the \emph{daily} return, returns beyond 2 weeks vary substantially
	\item Rebalancing is requied by the fund each day to ensure that the daily return can be achieved
\end{itemize}
\end{frame}
...
\begin{frame}{Liqidity and Leverage}
	One of the great attractions of ETF is their \emph{liqudity}
\begin{itemize}[<+-| alert@+>]
		\pause
...
\begin{frame}{How will liquidity and leverage respond in a crisis?}
\begin{itemize}[<+-| alert@+>]
	\pause
	...
	\item corporate bond ETF are seen as being particularly vunlerable
		\end{itemize}
	\item There is a concern that leverage can propogate a financial shock
		\begin{itemize}
	\item leverage can make funds vlunerable
...
\begin{itemize}[<+-| alert@+>]
	...
	\item The link between premium and liquidity is negative for the US and postitive for international funds
\end{itemize}
\pause
...
\begin{itemize}[<+-| alert@+>]
		\pause
	\item Emerging ETF have wider dispursion of premia and discount so the average tends to be low
...
\begin{itemize}[<+-| alert@+>]
	...
	\item This may be partly asynchromous activity 
		\begin{itemize}
...
\begin{frame}{Market efficiency}
	...
	\pause
\begin{itemize}[<+-| alert@+>]
	\item Bond and Garcis (2016) suggest that retail investors are \emph{noise traders} and that their migration to index funds will increase efficiency and provide fewer oppportunities for active managers
...
\begin{frame}{Evidence on Price Discoery}
\begin{itemize}[<+-| alert@+>]
	\item Glosten, Nallareddy and Zou (2016) find that stocks incporporate information more quickly once they are in an ETF
	\item Werners and Xue (2015) successfully test their assetion that price movement caused by ETF is informed (permanent) while price movement led by securities is noise (transitory)
	\item Lee and Shridharan (2017) find that stocks owned b ETF have higher trading costs, high comovement with index and receive less analyst coverage
\end{itemize}
\end{frame}
...
\begin{frame}{Corporate Governance}
	...
	\pause
\begin{itemize}[<+-| alert@+>]
	\item It is argued that passive holders of stock do not disciplimne management
		\begin{itemize}
...
\begin{frame}{Experiments with Corporate Governance}
	...
	\pause
\begin{itemize}[<+-| alert@+>]
	\item Boone and White (2015) find that firms wtih more passive institutional investment also increase their transparency of reporting
	\item Appel, Gormely and Keim (2016) find positive effect on poison pills, restricctions on shareholds and fewer dual-class share structures
\end{itemize}
\end{frame}

\begin{frame}{Crisis Performane}
\begin{block}{}
There is concern that liquidity may disappear at times of crisis. There are two case studie to assess
\end{block}
\pause
\begin{itemize}[<+-| alert@+>]
...
\begin{itemize}[<+-| alert@+>]
	\item The fragmentation of markets has increased the risk of liqudity gaps
\item DJIA Lost 998.5 (9\%) and recovered almost as swiftly
	...
	\item Uncertainty about the quoted prices for the underlyhing assets
...
\begin{itemize}[<+-| alert@+>]
		\pause
	\item They average draw-down was 0.24 compared to 0.08 for other products
...
\begin{block}{}
There was an increase in US equity market volatility in February 2018 as rising US interest rates and some concern about inflation cofronted the positive effect of US tax cuts
\end{block}
\pause
...
\begin{frame}{ESG}
	There has been increased attention to Environmental, Social and Governance issues in the investment community.  This appears to come from three main souces
	\pause 
\begin{itemize}[<+-| alert@+>]
	...
	\item Experience of ESG-related losses
		\begin{itemize}
			...
			\item Volkswagen emmissions
		\end{itemize}
	\item Perceptiokn that \emph{millenials} will have a much greater focus on ESG issues as they become more prominent in the investmetn industry
\end{itemize}
\end{frame}
...
\begin{itemize}[<+-| alert@+>]
	...
	\item Grossman and Stiglitz
	\item Malkiel (1995), Gruber (1996), Wermers (2000, 2003), and Jones
and Wermers (2011)
\item Charupat and Miu (2011) Multiple ETF
\end{itemize}
\end{frame}
