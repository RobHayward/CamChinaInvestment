...
\tikzstyle{block} = [rectangle, rounded corners, minimum width=3cm, 
  minimum height= 2cm, text centered, text width = 2.8cm, draw = black, fill = MyBrown!30]
...
\textbf{Ticker} & \textbf{Description} & \textbf{Cost (bp)}\\
\hline
...
\begin{figure}
\begin{tikzpicture}
%\scope[transform canvas={scale=0.9}]
...
\begin{frame}{Will liquidity remain in a crisis?}
	There is a concern that liquidity will disappear in a crisis
	\pause
\begin{itemize}[<+-| alert@+>]
	\item Bond funds are particularly vulnerable
	\item 
\end{itemize}
\end{frame}
...
\begin{itemize}[<+-| alert@+>]
	...
	\item These ETF and underlying securities that are usually very liquid
\end{itemize}
\end{frame}
