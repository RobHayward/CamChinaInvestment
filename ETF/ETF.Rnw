...
\begin{itemize}[<+-| alert@+>]
\pause
\item Hedge and other Alternative funds
\begin{itemize}
...
% does this go with the ETF explanation. 
...
\section{Reasons for ETF growth}
\begin{frame}{Basic ETF structure}
\begin{itemize}[<+-| alert@+>]
	\item They are can set up as unit trusts or mutual funds
		\begin{itemize}
			\item mutual funds allow reinvestment of dividends and loaning of securities and assets
		\end{itemize}
	\item 
\end{itemize}
\end{frame}
...
\begin{itemize}[<+-| alert@+>]
	\item Open-ended: 
		\begin{itemize}
			\item pooled investment vehicle to hold assets
			\item new shares can be created
			\item secondary market on closing price
		\end{itemize}
	\item Closed-ended
		\begin{itemize}
			\item pooled investment vehicle to hold assets
			\item no new shares can be created
			\item can be traded at all times
		\end{itemize}
\end{itemize}
\end{frame}
...
\pause
\begin{itemize}[<+-| alert@+>]
	\item An \emph{Authorised Participant} will deliver the underlying assets to the ETF in exchange for shares
		\begin{itemize}
			\item between 50,000 and 200,000 are usually creeated - 75,000 seems to be the most frequent
		\end{itemize}
...
\begin{frame}{Standard ETF structure}
%\resizebox{4cm}{!}{
\begin{center}
\begin{figure}
...
\begin{frame}{Different Types}
	...
	\pause
\begin{itemize}[<+-| alert@+>]
	\item Standard ETF: the Authorised Participant will exchange underlying securities or assets for cash
		\begin{itemize}
			\item these securities or assets form the assets of the ETF
			\item the authorised participant can arbitrage the fund and assets if there is a divergence in price
		\end{itemize}
	\item Synthetic ETF: the Authorised Participant will provide cash in exchange for shares
		\begin{itemize}
			...
			\item artbiotrage is more complex
...
\begin{frame}{Price discovery}
	The structure of the ETF is designed to ensure that there is pricing efficiency. However, (as we have seen) there are divergences between the values of underlying assets
\begin{itemize}[<+-| alert@+>]
		\pause
	\item Liquidity
...
\begin{itemize}[<+-| alert@+>]
	...
	\item Divergence is greater for synthetic relative to \emph{plain vanilla}
\end{itemize}
\end{frame}
...
\begin{itemize}[<+-| alert@+>]
		\pause
	\item Emerging ETF have wider sipursion of premia and discount so the average tends to be low
	\item The relatinship between liquidity and pricing may be non-linear (it may be U shaped)
...
\begin{frame}{Performance Under Fire}
	There is concern about ETF ability to function in a crisis. We have two cases that provide some informaton:
	\pause
\begin{itemize}[<+-| alert@+>]
	\item The \empn{Flash Crash} of 6 May 2010
	\item The \emph{Volatility shock} of FEbruary 2018
\end{itemize}
\end{frame}
...
\begin{itemize}[<+-| alert@+>]
	...
	\item at 2.32 6 May 2011 there was a sale of 75,000 e-mini contracts
...
\begin{itemize}{iShares Russell Growth}
\end{frame}
...
\begin{frame}{Investment universe}
	\begin{itemize}
		\item A spectrum of investment behicles
		\item Two poles: active and passive
		\item Three poles: active, hybride and passive
	\end{itemize}
\end{frame}
...
\item L.F. Ackert and Y.S. Tian, 2008, Arbitrage, Liquidity and Valuation of Exchange Traded Funds, Financial Markets, Institutions and Instruments, volume 17,. No. 5, pp 331-363. 
